...
cv_iid <- sdmTMB_cv(
  ...
  control = sdmTMBcontrol(
    start = list(re_cov_pars = matrix(10, 1, 1)), # weak SD penalty on random intercept
...
The log predictive density differences tell us about predictive performance. The SE accounts for variability across folds. As a rule of thumb, differences exceeding about 2 SE are generally considered meaningful.
...
5.  Is there a natural experiment we could use to evaluate the see-saw effect for one year? 6 How might we design a simulation experiment to choose a model?
...
-   If you have to, simulation testing can help answer thorny questions
