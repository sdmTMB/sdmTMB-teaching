...
We start by constructing an SPDE mesh with fmesher. This creates some matrices that are used internally when fitting the model. We will use the shortcut function `make_mesh()` and use a cutoff (minimum triangle length of 10 km). Note: this is the only parameter that can be changed in `make_mesh()` but more control over mesh arguments can be done with fmesher itself.
...
```{r}
cv_intercept$fold_loglik
cv_depth$fold_loglik

# check each fold between the two models:
cv_depth$fold_loglik - cv_intercept$fold_loglik
```

These are the log predictive densities for all left out data combined.
...
1.  Which model does the log predictive density suggest has better out-of-sample predictive performance?
...
We could also use the `ggeffects` package. For more details see the [visualizing marginal effects vignette](https://pbs-assess.github.io/sdmTMB/articles/ggeffects.html).

```{r}
ggeffects::ggpredict(fit, "log_depth [all]") |> plot()
```

# Prediction

Let's now predict on a grid that covers the entire survey (`wcvi_grid`). This object is built into the sdmTMB package.

We can use the `sdmTMB::replicate_df()` function to replicate our survey grid for each year:

```{r}
grid <- replicate_df(
  wcvi_grid, time_name = "year", 
  time_values = unique(dat$year)
)
# add back any missing predictors:
grid$log_depth <- log(grid$depth)
grid$year_factor <- factor(grid$year)
head(grid)
```
